...
import { staffCabinetTempHumApi } from '@/lib/staffApi/cabinetApi';
import { staffReportApi } from '@/lib/staffApi/reportApi';
import type {
  StaffCabinetTempHumCabinet,
  StaffCabinetTempHumChartData,
...
function CabinetChartBlock({
  ...
  cabinet: StaffCabinetTempHumCabinet;
  year: number;
  month: number;
}) {
  const [data, setData] = useState<StaffCabinetTempHumChartData | null>(null);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        setLoading(true);
        const response = await staffCabinetTempHumApi.getChart({
...
export default function CabinetTempHumChartCard() {
  ...
  const [cabinets, setCabinets] = useState<StaffCabinetTempHumCabinet[]>([]);
  const [loading, setLoading] = useState(true);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        setLoading(true);
        const response = await staffCabinetTempHumApi.getOverview({ year, month });
        if (!cancelled && response.success && response.data) {
  ...
  const exportExcel = async () => {
    try {
      ...
      await staffReportApi.downloadCabinetTempHumExcel({ year, month });
    } catch (error) {
  ...
  const exportPdf = async () => {
    try {
      ...
      await staffReportApi.downloadCabinetTempHumPdf({ year, month });
    } catch (error) {
